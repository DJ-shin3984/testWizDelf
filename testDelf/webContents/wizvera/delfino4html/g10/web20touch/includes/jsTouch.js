...
* -- Developer - neochoon@wizvera.com
*/

/**
 * 한 html에 여러 페이지 div들을 처리할수있도록 수정
 */
var jsTouchHashes = {};

var ajaxGet = function(url, data, success) {
    if (url.indexOf('http') === 0) {
        $.get(url, data, function() {success.apply(undefined, Array.prototype.slice.call(arguments, 0)); });
    } else {
        var id = url.charAt(0) == '#' ? url.substring(1) : url;
        var page = jsTouchHashes[id];
        var data = page.html;
        var onload = page.onload || function() {};
        if (data) {
            success(data);
            onload(id);
        }
...
var loadPages = function() {
    var pages = $('div.jsTouchPanel');
    for (var i = 0; i < pages.length; i++) {
        var page = $(pages[i]);
        var id = page.attr('id');
        jsTouchHashes[id] = { html : page.html() };
        page.remove();
    }
}

window.addEventListener('DOMContentLoaded', loadPages, false);
...
var jsTouch = {
	...
	init: function(name, params) {
        if (!params) {
            params = name;
            name = params.page.charAt(0) == '#' ? params.page.substring(1) : params.page;
        }
        $(document.body).append('<div id="'+name+'" class="jsTouchPanel" style="position: absolute; left: 0px; top: 0px; border-left: 0px !important;"></div>');
	...
	loadPage: function(url, params, callBack) {
        var event = window.event || this.event;
		if (event) { var currObj = this.getCurrentBox(event.target); }
	...
	overlayPage: function(url, params, callBack) {
		...
		ajaxGet(url, {}, function (data) {
			...
			if (isToolbar) $('div.overlay div.content').css('top', jsTouch.contentTop);
...
function jsTouchBox(name, params) {
	...
	function jsTouch_animate(HTML, transition) {
		...
		div_old && $(div_old).attr('aria-hidden', true);
		div_new && $(div_new).attr('aria-hidden', false);
		div_new && $(div_new).removeAttr('aria-hidden');
		setTimeout(function() {
			div_old && $(div_old).attr('aria-modal', false);
			div_old && $(div_old).removeAttr('aria-modal');
			div_new && $(div_new).attr('aria-modal', true);
		}, 500);
		switch (transition) {
			case 'slide-left':
				// init divs
	...
	function jsTouch_resize() {
		...
		if (isToolbar) $(div).css('top', jsTouch.contentTop);
		if (isFooter) $(div).css('bottom', '80px');
		...
		// -- set scroll to 0, 0 (in the browser it will hide url bar) -> doesn`t work from ios 7.
		//window.scrollTo(0, 0);
		//console.log('In jsTouch this : '+this.width+', '+this.height+'... window : '+window.innerWidth+', '+window.innerHeight);
	}
...
window.applicationCache && window.applicationCache.addEventListener && window.applicationCache.addEventListener('updateready', function(){ window.applicationCache.swapCache(); }, false);
