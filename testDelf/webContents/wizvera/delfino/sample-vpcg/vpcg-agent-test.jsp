...
<%!
   public Proxy getProxy(String proxyIp, int proxyPort) {
      Proxy proxy = null;
      if (proxyIp != null && !"".equals(proxyIp)) proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(proxyIp, proxyPort));
      return proxy;
   }
%>
<%!
	...
	//String vpcgResultUrl = "https://test.wizvera.com:8443/vpcg/delfino_vpcgResult.jsp";
	//String vpcgResultUrl = delfinoConfig.getVpcgResultUrl();
	//String vpcgAccessToken = delfinoConfig.getVpcgAccessToken();
    //VpcgSignResultService vpcgSignResultService = new VpcgSignResultServiceAgentLite(vpcgResultUrl, 20000, 10000);
      
    String vpcgResultServiceUrl = delfinoConfig.getVpcgResultUrl();
    //String vpcgResultServiceUrl = "http://192.168.0.193:8001/v1/sign/result";
    int connectionTimeoutMillis = delfinoConfig.getVpcgConnectionTimeout();
    int readTimeoutMillis = delfinoConfig.getVpcgReadTimeout();
    String accessToken = delfinoConfig.getVpcgAccessToken();
    String sharedSecret = delfinoConfig.getVpcgSharedSecret();
    String gatewayUrl = delfinoConfig.getVpcgGatewayUrl();
    String proxyIp = delfinoConfig.getVpcgProxyIp();
    int proxyPort = delfinoConfig.getVpcgProxyPort();
    VpcgSignResultServiceAgentLite.Config config = VpcgSignResultServiceAgentLite.config().vpcgResultServiceUrl(vpcgResultServiceUrl).connectionTimeoutMillis(connectionTimeoutMillis).readTimeoutMillis(readTimeoutMillis).accessToken(accessToken).sharedSecret(sharedSecret).gatewayUrl(gatewayUrl).proxy(getProxy(proxyIp, proxyPort));
    VpcgSignResultService vpcgSignResultService = new VpcgSignResultServiceAgentLite(config);
...
function fnSignResult() {
    ...
    if (confirm("Delfino SDK를 이용한 서명검증을 하시겠습니까?")) {
	    ...
	    frm.action = "../demo20/signResultVpcg.jsp?_action=TEST_VPCG";
    }
...
<script type="text/javascript" language="javascript">
    ...
    var VPCG_SERVICE = "../svc/delfino_vpcgAgent.jsp?origin=" + encodeURIComponent(origin);
