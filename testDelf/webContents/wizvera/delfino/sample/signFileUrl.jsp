...
<body>
	...
	<form name="form1">
		...
		PKCS7 다운로드 URL : <input type="text" name="pkcs7downloadurl" value="/delfino-demo/delfino/sample/tbsFileDownload.jsp?file=data.pdf.sig" style="width:400px;"/><br/>
	...
	function callSignFileUrlAddDown(saveType, del){
		Delfino.signFileUrlDown(document.form1.pkcs7downloadurl.value, {dataType:'PKCS7', saveType:saveType, deleteFile:del, setCookie:'test4=2345'}, down_complete);
	}
	function callSignFileUrlAddSign(saveType, del, multi, arrayData){
		if(multi){
			if(arrayData){
				Delfino.signFileUrlSign(document.form2.downResult.value.split("|"), {dataType:'PKCS7', signType:'sig', saveType:saveType, deleteFile:del, multiSign:multi, multiSignDelimiter:'|'}, sign_complete);	
			}else{
				Delfino.signFileUrlSign(document.form2.downResult.value, {dataType:'PKCS7', signType:'sig', saveType:saveType, deleteFile:del, multiSign:multi, multiSignDelimiter:'|'}, sign_complete);
			}
		}else{
			var arr = new Array();
			arr = document.form2.downResult.value.split("|");	
	...
	function callSignFileUrlUp(saveType, del, arrayData){
		...
		if(arrayData){
			Delfino.signFileUrlUp(document.form2.signResult.value.split("|"), document.form1.uploadurl.value, {saveType:saveType, deleteFile:del, delimiter:'|'}, upload_complete);
		}else{
	...
	<br/><br/>
	<button onclick="callSignFileUrlAddDown('default',false);">다운로드(디폴트, PKCS7)</button><br/>
	<button onclick="callSignFileUrlAddDown('default',true);">다운로드(디폴트/옵션delete, , PKCS7)</button>(다운로드 받기전에 default폴더를 지운후 다운로드를 받음)<br/>
	<button onclick="callSignFileUrlAddDown('select',false);">다운로드(유저선택, PKCS7)</button><br/>
	<h4>파일서명(서명)</h4>
	...
	<br/><br/>	
	<button onclick="callSignFileUrlAddSign('default',false, false);">서명(디폴트, addSign)</button><br/>
	<button onclick="callSignFileUrlAddSign('default',true, false);">서명(디폴트/옵션delete, addSign)</button>(서명후에 원본 파일을 지움)<br/>
	<button onclick="callSignFileUrlAddSign('select',false, false);">서명(유저선택, addSign)</button><br/>
	<button onclick="callSignFileUrlAddSign('select',true, false);">서명(유저선택/옵션delete, addSign)</button>(서명후에 원본 파일을 지움)<br/>
	<br/><br/>	
