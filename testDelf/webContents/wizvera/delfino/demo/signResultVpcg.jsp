...
//             signVerifierResult = signVerifier.verifyPKCS7WithUserConfirm(pkcs7, expectedUserConfirmFormat, certStatusCheckType);
            signVerifierResult = signVerifier.verifyPKCS7WithUserConfirm(pkcs7, expectedUserConfirmFormat, 1);
            
            out.println("<br/><br/>!@# !@# : "+signVerifierResult.getClass()+"<br/><br/>");
            out.println("<br/>!@# certStatusCheckType: "+certStatusCheckType);
            out.println("<br/>!@# signVerifierResult: "+signVerifierResult);
            
            String account = signVerifierResult.getSignedParameter("account ");
            String amount = signVerifierResult.getSignedParameter("amount");
            String recvUser = signVerifierResult.getSignedParameter("recvUser");

            out.println("<br/>!@# account: "+account);
            out.println("<br/>!@# amount: "+amount);
            out.println("<br/>!@# recvUser: "+recvUser+"<br/><br/>");
        ...
        ***************************************************************/
        ...
        if ("TEST_registerCert".equals(action)) {
            ...
            if (isVerifiableVid) { //Vid를 이용한 본인확인 가능(공동인증서, 금융인증서)
        ...
        ***************************************************************/
        ...
        //TODO: 핀테크인증서일경우 saveTxInfo: 내부테스용입니다.
        String vpcgMode = com.wizvera.WizveraConfig.getDefaultConfiguration().getVpcgMode();
        String vpcgResultUrl = com.wizvera.WizveraConfig.getDefaultConfiguration().getVpcgResultUrl();
        if (isPrivateCert && !("agent".equals(vpcgMode) && vpcgResultUrl.indexOf(".certgate.io")>0)) {
            ...
            out.println("<br/>-. signVerifierResult.saveTxInfo: <b>" + isSaveTxInfo + "</b>");
        } else {
            out.println("<br/>-. signVerifierResult.saveTxInfo: <b>SKIP</b>");
        }

        //TODO: delete
        out.println("<br/>-. TEST: signVerifierResult.getDelfinoNonce(): [" + signVerifierResult.getDelfinoNonce() + "]");
